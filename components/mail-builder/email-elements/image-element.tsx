...
export function ImageElement({ id, properties = {}, isSelected, onSelect, onUpdate }: ImageElementProps) {
  const [showUpload, setShowUpload] = useState(!properties.src)
  const [imageUrl, setImageUrl] = useState<string>(properties.src || "")
  const [altText, setAltText] = useState(properties.alt || "")
  const [isConfiguring, setIsConfiguring] = useState(false)
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type.startsWith("image/")) {
      ...
      setUploading(true)
      try {
        // Upload file using the file store
        const uploadedFile = await uploadFile(workspaceId, file, null)
        
        if (uploadedFile && uploadedFile.cdnUrl) {
          const uploadedUrl = uploadedFile.cdnUrl || uploadedFile.directUrl || ""
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (imageFile) {
      ...
      setUploading(true)
      try {
        const uploadedFile = await uploadFile(workspaceId, imageFile, null)
      ...
      {showUpload || !properties.src ? (
        <div className="space-y-3" onDragOver={handleDragOver} onDrop={handleDrop}>
          ...
          <div className="space-y-2">
            ...
            />
            <div className="flex gap-2">
              <Button size="sm" onClick={handleImageUpload} disabled={!imageUrl || uploading}>
                <Link className="w-3 h-3 mr-1" />
                Add Image
              </Button>
        ...
        <div className="space-y-3">
          ...
          {isConfiguring && (
            <div className="space-y-2 p-2 bg-muted rounded">
              ...
              <div>
                <label className="text-xs font-medium">Opacity: {properties.opacity || 100}%</label>
                <Slider
                  value={[properties.opacity || 100]}
                  onValueChange={([value]) => onUpdate?.({ ...properties, opacity: value })}
                  max={100}
                  min={0}
                  step={5}
                />
              ...
              <div>
                <label className="text-xs font-medium">Rotation: {properties.rotation || 0}°</label>
                <Slider
                  value={[properties.rotation || 0]}
                  onValueChange={([value]) => onUpdate?.({ ...properties, rotation: value })}
                  max={360}
                  min={-360}
                  step={15}
                />
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value: "left" | "center" | "right") => onUpdate?.({ ...properties, alignment: value })}
                >
                  <SelectTrigger className="h-8">
                    <SelectValue />
                  </SelectTrigger>
