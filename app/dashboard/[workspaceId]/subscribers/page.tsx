...
import { Search, Plus, Mail, Trash2, ChevronRight, Send, X, Copy, Loader2, Users, UserCheck, UserX, AlertCircle } from 'lucide-react';
...
export default function SubscribersPage() {
  ...
  console.log(subscribersList, 'subscribers list');
  console.log(stats?.active, 'sub stats');
  ...
  const sendBulkMessage = () => {
    if (!bulkMessage.trim() || selectedSubscribers.length === 0) return;
    
    // Get selected subscriber emails safely
    ...
    // Store in localStorage for the email composer
    ...
    router.push(`/dashboard/${workspaceId}/email/composer?bulk=true&count=${selectedEmails.length}`);
  };
        ...
        <div className='flex w-full justify-between items-center'>
          <div>
            ...
            <p style={{ color: '#666666' }}>
              Manage all your email subscribers
            </p>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Button asChild>
              <Link href={`/subscribe/${workspaceId}`} target="_blank">
            ...
            <table className="w-full">
              <thead>
                <tr style={{ backgroundColor: '#F5F5F5', borderBottom: '1px solid #E5E5E5' }}>
                  ...
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-bold" style={{ color: '#666666' }}>Name</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-bold" style={{ color: '#666666' }}>Email</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-bold" style={{ color: '#666666' }}>Status</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-bold" style={{ color: '#666666' }}>Joined</th>
                  <th className="px-4 sm:px-6 py-3 text-right text-xs font-bold" style={{ color: '#666666' }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  >
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <Link href={`/dashboard/${workspaceId}/subscribers/${subscriber.id}`}>
                        <p className="font-medium hover:text-red-600 capitalize transition-colors" style={{ color: '#1A1A1A' }}>
                          {subscriber.name || '—'}
                        </p>
                      </Link>
                    </td>
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <span 
                        ...
                        style={{
                          backgroundColor: 
                            subscriber.status === 'ACTIVE' ? '#E6F7E6' :
                            subscriber.status === 'UNSUBSCRIBED' ? '#F5F5F5' : '#FFE6E6',
                          color:
                            subscriber.status === 'ACTIVE' ? '#10B981' :
                            subscriber.status === 'UNSUBSCRIBED' ? '#6B7280' : '#DC143C'
                        }}
                      >
                        {subscriber.status}
                      </span>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 p-3 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Note:</strong> Only active subscribers will receive emails. 
              ...
              <div className="flex gap-3">
                ...
                <Button
                  ...
                  className="flex-1"
                  style={{ 
                    backgroundColor: bulkMessage.trim() ? '#DC143C' : '#CCCCCC'
                  }}
                >
                  <Send size={18} className="mr-2" />
                  Continue
...
// import { Button } from '@/components/ui/button';
// import { Input } from '@/components/ui/input';
// import { Search, Plus, Mail, MessageSquare, Trash2, ChevronRight, Send, X, Copy, Loader2, Users, UserCheck, UserX, AlertCircle } from 'lucide-react';
// import { useWorkspaceStore } from '@/lib/stores/workspace'
// import { toast } from 'sonner';
// import { useRouter } from 'next/navigation';
// import { useSubscriberStore } from '@/lib/stores/subscriber/subscriber-store';
...
//   const [showBulkModal, setShowBulkModal] = useState(false);
//   const [bulkMessage, setBulkMessage] = useState('');
//   const [selectedChannel, setSelectedChannel] = useState<'sms' | 'email'>('email');
//   const [selectedSubscribers, setSelectedSubscribers] = useState<string[]>([]);
//   const [copied, setCopied] = useState(false);
//   const [statusFilter, setStatusFilter] = useState<'ALL' | 'ACTIVE' | 'UNSUBSCRIBED' | 'BOUNCED'>('ALL');
...
//   console.log(subscribers, ' sum man')
//   // console.log(subscribers, ' sum man')
...
//     // Status filter (already applied at API level, but double-check)
//     if (statusFilter !== 'ALL' && sub.status !== statusFilter) return false;
    
//     // Type filter (simplified - you might want to add a channels field)
//     if (filterType === 'email') return true; // All have email
//     if (filterType === 'sms') return false; // No SMS yet
...
//                       className="flex-1 px-4 py-2 rounded border capitalize font-medium text-sm"
//                       style={{
//                         borderColor: selectedChannel === ch ? '#DC143C' : '#E5E5E5',
//                         backgroundColor: selectedChannel === ch ? '#FFF5F5' : '#FFFFFF',
//                         color: selectedChannel === ch ? '#DC143C' : '#666666'
//                       }}
//                     >
//                       {ch}
//                     </button>
