...
export function TopUpModal({
  ...
  const [amountToTopup, setAmountToTopup] = useState<number>(0); // NEW: Separate state for balance amount
  ...
  if (serviceType === 'balance') {
    totalPrice = amountToTopup; // For balance, total price is the amount user entered
  } else {
    totalPrice = priceInfo ? (unitsToBuy * priceInfo.price) : 0;
  }
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (result.success && result.data) {
                ...
                // For non-balance services, set min purchase
                if (serviceType !== 'balance') {
                  ...
                  // For balance, set default amount
                  setAmountToTopup(500); // Default minimum amount
                }
            }
          }
        } catch (err) {
  ...
  const handlePurchase = async () => {
    ...
    if (serviceType === 'balance') {
      if (amountToTopup < 500) {
        toast.error(`Minimum top-up amount is ₦500`);
        return;
      }
      if (amountToTopup > 1000000) {
        toast.error(`Maximum top-up amount is ₦1,000,000`);
        return;
      }
    } else {
      if (!priceInfo) return;
    ...
    try {
      const response = await fetch(`/api/workspace/${workspaceId}/wallet/top-up`, {
        ...
        body: JSON.stringify({
          serviceType,
          quantity: serviceType === 'balance' ? amountToTopup : unitsToBuy,
          price: totalPrice,
              ...
              {!priceInfo && serviceType !== 'balance' ? (
                ...
                <>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setAmountToTopup(val);
                        }}
                        className="bg-background border-border text-foreground"
                        placeholder="Enter amount to top up"
                      />
                      <p className="text-[10px] text-muted-foreground">Minimum top-up is ₦500, Maximum is ₦1,000,000</p>
                    </div>
                  )}
                  ...
                  <div className="p-4 rounded-lg bg-primary/5 border border-primary/10">
                    ...
                    {serviceType === 'balance' && (
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-sm text-muted-foreground">Amount to top up:</span>
                        <span className="font-bold text-foreground">₦{amountToTopup.toLocaleString()}</span>
                      </div>
                    )}
            ...
            <DialogFooter>
              ...
              <Button 
                ...
                disabled={
                  isProcessing || 
                  (!priceInfo && serviceType !== 'balance') ||
                  (serviceType === 'balance' && amountToTopup < 500) ||
                  (serviceType !== 'balance' && (!priceInfo || unitsToBuy < (priceInfo?.minPurchase || 1)))
                } 
