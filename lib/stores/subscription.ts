      ...
      fetchSubscription: async () => {
        ...
        try {
          ...
          if (data.data?.subscription) {
            ...
            // The usage data is already in the subscription object from the API
            // No need to overwrite it with workspace store
            const enhancedSubscription: Subscription = {
              ...sub,
              // Ensure limits are set if not present
              limits: sub.limits || {
                sms: 0,
                email: 0,
                otp: 0,
              ...
              // Use the usage from the API response
              usage: sub.usage || {
                sms: 0,
                email: 0,
                otp: 0,
            ...
            console.log('✅ Enhanced subscription with usage:', enhancedSubscription.usage);
            set({ subscription: enhancedSubscription, isLoading: false });
          } else {
            set({ subscription: null, isLoading: false });
          }
